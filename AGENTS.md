...
3. **Open `QA_OPEN_BUGS.md`** for the curated list of QA-surfaced bugs that need `app/*` fixes. If you're picking up `app/*` work, this is the most direct way to know what's broken from a PM perspective. If a bug is in your domain (UI / sandbox / etc.), consider whether to land the fix before opening new work.
4. Inspect `git status`, `git diff`, and `git log --oneline -5` for the actual current state.
5. Open `MASTERPLAN.md` only when you need roadmap context (it is long).
