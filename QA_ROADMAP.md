...
| **QA-07b** | **Sandbox UI mutation — non-destructive flows.** Node property panel edit (title/duration/owner via the side-panel form), edge create via the dependency checkbox UI, cycle rejection at edit time, Tidy auto-layout button. Each is non-destructive (sandbox-only). Sandbox cleanup story: scoped to draft sandboxes per discovered project; subsequent runs reuse or recreate. | Picks up the next round of Codex Timeline planner UI mutations QA-07 explicitly deferred. | ~3h |
| **QA-07c** | **Sandbox UI Apply via the confirmation modal.** Destructive (mutates project_phases). Requires a real test-project lifecycle: scenario creates a fresh test project via HTTP, applies a sandbox through the UI modal, verifies the project's phases match, then deletes the test project. | Closes the last UI surface in the v1.4 Planning Sandbox — the bridge from sandbox to live timeline. | ~3h |
